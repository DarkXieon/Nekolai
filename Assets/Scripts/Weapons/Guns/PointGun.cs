...
public class PointGun : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        this._x0 = this.GetDistanceFromOrigin(_pointWith.position - _rotateAround.position);

        this._fliped = this._gunTransform.rotation.eulerAngles.x == 180 || this._gunTransform.rotation.eulerAngles.x == -180; //check the rotation of the gun, this should return false

        Debug.Assert(!_fliped, "The gun was fliped before it should have been"); //make sure the value is what it should be
    ...
    {
        var angle = Mathf.Atan((cursorCoordinates.y - _rotateAround.position.y) / (cursorCoordinates.x - _rotateAround.position.x));
